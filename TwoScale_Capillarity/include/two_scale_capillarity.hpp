...
TwoScaleCapillarity<dim>::TwoScaleCapillarity(const xt::xtensor_fixed<double, xt::xshape<dim>>& min_corner,
  ...
  sigma(sim_param.sigma), mod_grad_alpha1_bar_min(sim_param.mod_grad_alpha1_bar_min),
...
void TwoScaleCapillarity<dim>::perform_Newton_step_relaxation(std::unique_ptr<State> local_conserved_variables,
  ...
  if(!std::isnan(H_bar_loc)) {
    ...
    try {
      p2_minus_p1_times_h = rho1_loc/(1.0 - alpha1_bar_loc)*
    ...
    const auto fac_Ru                = sigma*(3.0*H_lim/(kappa*rho1d_loc))*(rho1_loc/(1.0 - alpha1_bar_loc)) -
...
void TwoScaleCapillarity<dim>::execute_postprocess(const double time) {
  ...
  samurai::for_each_cell(mesh,
                         [&](const auto& cell)
                         {
                           ...
                           typename Field::value_type p2_minus_p1_times_h;
                           try {
                             p2_minus_p1_times_h = rho1/(1.0 - alpha1_bar[cell])*
