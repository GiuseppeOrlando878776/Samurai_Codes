...
namespace samurai {
  ...
  private:
     ...
     * @param grad_alpha1_bar_L left state
     * @param grad_alpha1_bar_R right state
     * @param curr_d current direction
     */
    FluxValue<cfg_st> compute_discrete_flux(const auto& grad_alpha1_bar_L,
                                            const auto& grad_alpha1_bar_R,
  ...
  SurfaceTensionFlux<Field, Field_Vect>::compute_discrete_flux(const auto& grad_alpha1_bar_L,
                                                               const auto& grad_alpha1_bar_R,
                                                               const std::size_t curr_d) {
    return static_cast<Number>(0.5)*
           (this->template evaluate_surface_tension_operator<Field_Vect>(grad_alpha1_bar_L, curr_d) +
            this->template evaluate_surface_tension_operator<Field_Vect>(grad_alpha1_bar_R, curr_d));
  }
