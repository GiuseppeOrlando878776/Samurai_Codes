...
namespace samurai {
  ...
  //
  template<class Field>
  HLLCFlux_Conservative<Field>::HLLCFlux_Conservative(const SG_EOS<Number>& EOS_phase1_,
  ...
  //
  template<class Field>
  FluxValue<typename HLLCFlux_Conservative<Field>::cfg>
  HLLCFlux_Conservative<Field>::compute_middle_state(const FluxValue<cfg>& q,
                                                     const Number S,
                                                     const Number S_star,
    ...
    const auto p1_star           = (p1 + this->EOS_phase1.get_pi_infty())*
                                   ((this->EOS_phase1.get_gamma() - static_cast<Number>(1.0))*rho1 -
                                    (this->EOS_phase1.get_gamma() + static_cast<Number>(1.0))*rho1_star)/
                                   ((this->EOS_phase1.get_gamma() - static_cast<Number>(1.0))*rho1_star -
                                    (this->EOS_phase1.get_gamma() + static_cast<Number>(1.0))*rho1)
                                 - this->EOS_phase1.get_pi_infty();
    q_star(ALPHA1_RHO1_e1_INDEX) = m1_star*this->EOS_phase1.e_value(rho1_star, p1_star);

    const auto rho2_star         = m2_star/(static_cast<Number>(1.0) - alpha1); /*--- TODO: Add treatment for vanishing volume fraction ---*/
    const auto p2_star           = (p2 + this->EOS_phase2.get_pi_infty())*
                                   ((this->EOS_phase2.get_gamma() - static_cast<Number>(1.0))*rho2 -
                                    (this->EOS_phase2.get_gamma() + static_cast<Number>(1.0))*rho2_star)/
                                   ((this->EOS_phase2.get_gamma() - static_cast<Number>(1.0))*rho2_star -
                                    (this->EOS_phase2.get_gamma() + static_cast<Number>(1.0))*rho2)
  ...
  //
  template<class Field>
  void HLLCFlux_Conservative<Field>::compute_discrete_flux(const FluxValue<cfg>& qL,
                                                           const FluxValue<cfg>& qR,
                                                           std::size_t curr_d,
                                                           FluxValue<cfg>& F_minus,
