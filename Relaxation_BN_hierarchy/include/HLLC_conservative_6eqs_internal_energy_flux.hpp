...
namespace samurai {
  ...
  public:
    HLLCFlux_Conservative(const SG_EOS<typename Field::value_type>& EOS_phase1_,
  ...
  //
  template<class Field>
  HLLCFlux_Conservative<Field>::HLLCFlux_Conservative(const SG_EOS<typename Field::value_type>& EOS_phase1_,
  ...
  auto HLLCFlux_Conservative<Field>::compute_middle_state(const FluxValue<typename Flux<Field>::cfg>& q,
    ...
    const auto rho1_star         = m1_star/alpha1; /*--- TODO: Add treatment for vanishing volume fraction ---*/
    const auto p1_star           = (p1 + this->EOS_phase1.get_pi_infty())*
                                   ((this->EOS_phase1.get_gamma() - static_cast<typename Field::value_type>(1.0))*rho1 -
                                    (this->EOS_phase1.get_gamma() + static_cast<typename Field::value_type>(1.0))*rho1_star)/
                                   ((this->EOS_phase1.get_gamma() - static_cast<typename Field::value_type>(1.0))*rho1_star -
                                    (this->EOS_phase1.get_gamma() + static_cast<typename Field::value_type>(1.0))*rho1)
                                 - this->EOS_phase1.get_pi_infty();
    q_star(ALPHA1_RHO1_e1_INDEX) = m1_star*this->EOS_phase1.e_value(rho1_star, p1_star);

    const auto rho2_star         = m2_star/(static_cast<typename Field::value_type>(1.0) - alpha1); /*--- TODO: Add treatment for vanishing volume fraction ---*/
    const auto p2_star           = (p2 + this->EOS_phase2.get_pi_infty())*
                                   ((this->EOS_phase2.get_gamma() - static_cast<typename Field::value_type>(1.0))*rho2 -
                                    (this->EOS_phase2.get_gamma() + static_cast<typename Field::value_type>(1.0))*rho2_star)/
                                   ((this->EOS_phase2.get_gamma() - static_cast<typename Field::value_type>(1.0))*rho2_star -
                                    (this->EOS_phase2.get_gamma() + static_cast<typename Field::value_type>(1.0))*rho2)
                                 - this->EOS_phase2.get_pi_infty();
    q_star(ALPHA2_RHO2_e2_INDEX) = m2_star*this->EOS_phase2.e_value(rho2_star, p2_star);
