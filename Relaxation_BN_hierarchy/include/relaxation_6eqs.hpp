...
private:
  ...
  void get_min_alpha(const typename Field::value_type time); /*--- Get track of the minimum value of volume fraction ---*/
                         ...
                         [&](const auto& cell)
                            {
                              ...
                              conserved_variables[cell][ALPHA2_RHO2_INDEX] = (static_cast<typename Field::value_type>(1.0) -
                         ...
                         [&](const auto& cell)
                            {
                              ...
                              p1[cell]              = p1_loc;
                              const auto c1_loc     = EOS_phase1.c_value(rho1_loc, p1_loc);
                              c1[cell]              = c1_loc;
                              ...
                              for(std::size_t d = 0; d < Field::dim; ++d) {
                                local_res = std::max(std::abs(vel[cell][d]) + c_loc,
                                                     local_res);
                              }
                            }
                        );
...
//
template<std::size_t dim>
void Relaxation<dim>::get_min_alpha(const typename Field::value_type time) {
  /*--- Loop over all cells to compute the minimum value of alpha1 ---*/
  auto alpha1_min = static_cast<typename Field::value_type>(1.0);
...
// Update auxiliary fields after solution of the system for the output
//
template<std::size_t dim>
...
void Relaxation<dim>::update_pressure_before_relaxation() {
  /*--- Update variables with a loop over all cells ---*/
  samurai::for_each_cell(mesh,
                         [&](const auto& cell)
                         {
                           // Pre-fetch variables that will be used several times so as to exploit possible vectorization
                           ...
                           auto e1_0_loc = m1E1_loc/m1_loc; /*--- TODO: Add treatment for vanishing volume fraction ---*/
                           auto e2_0_loc = m2E2_loc/m2_loc; /*--- TODO: Add treatment for vanishing volume fraction ---*/
                           ...
                           const auto rho1_loc = m1_loc/alpha1_loc; /*--- TODO: Add treatment for vanishing volume fraction ---*/
                           rho1[cell]          = rho1_loc;
                           const auto p1_loc   = EOS_phase1.pres_value(rho1_loc, e1_0_loc);
                           p1[cell]            = p1_loc;
                         ...
                         [&](const auto& cell)
                            {
                              ...
                              const auto Laplace_cst_1 = (p1_loc + EOS_phase1.get_pi_infty())/
                                                         std::pow(rho1_loc, EOS_phase1.get_gamma());
                              /*const auto Laplace_cst_2 = (p2_loc + EOS_phase2.get_pi_infty())/
                              ...
                              while(nite < max_iters &&
                                ...
                                rho2_loc = m2_loc/alpha2_loc; /*--- TODO: Add treatment for vanishing volume fraction ---*/

                                p1_loc            = std::pow(rho1_loc, EOS_phase1.get_gamma())*Laplace_cst_1 - EOS_phase1.get_pi_infty();
                                const auto e2_loc = (e_0 - Y1_0*EOS_phase1.e_value(rho1_loc, p1_loc))/Y2_0;
                                p2_loc            = EOS_phase2.pres_value(rho2_loc, e2_loc);

                                /*p2_loc            = std::pow(rho2_loc, EOS_phase2.get_gamma())*Laplace_cst_2 - EOS_phase2.get_pi_infty();
                                const auto e1_loc = (e_0 - Y2_0*EOS_phase2.e_value(rho2_loc, p2_loc)))/Y1_0;
                                p1_loc             = EOS_phase1.pres_value(rho1_loc, e1_loc);*/
