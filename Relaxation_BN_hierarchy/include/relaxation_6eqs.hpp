...
private:
  ...
  void get_min_alpha(const Number time); /*--- Get track of the minimum value of volume fraction ---*/
...
//
template<std::size_t dim>
void Relaxation<dim>::get_min_alpha(const Number time) {
  /*--- Loop over all cells to compute the minimum value of alpha1 ---*/
  auto alpha1_min = static_cast<Number>(1.0);
  ...
  while(t != Tf) {
    ...
    // Consider the second stage for the second order
      ...
      samurai::update_ghost_mr(conserved_variables);
        ...
        conserved_variables_tmp = conserved_variables - dt*Cons_Flux - dt*NonCons_Flux;
      #endif
      conserved_varaibles_np1.resize();
