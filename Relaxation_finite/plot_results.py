...
def line_plot(ax, x, y):
    ...
    if args.reference is not None:
        if args.column_reference is None:
            sys.exit("Unknown column to be read")
    ...
    #Add legend
    if args.reference is not None:
       ax.legend([plot_ref, plot_samurai], ['Reference results', 'Samurai results'], fontsize="20", loc="best")
...
parser.add_argument('--column_reference', type=int, required=False, help='column of reference results')
...
if args.save:
    if args.end is None:
        ...
        ani.save(args.save + '.gif', writer=writergif)
else:
    plt.show()
