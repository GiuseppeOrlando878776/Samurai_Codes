...
namespace samurai {
  ...
  FiniteRatePresTemp<Field>::make_relaxation() {
    ...
    /*--- Perform instantaneous velocity relaxation and then
          coupled finite-rate pressure and temperature ---*/
    relaxation_step.set_scheme_function([&](samurai::SchemeValue<cfg>& local_conserved_variables,
                                            const auto& cell, const auto& field)
                                            {
                                              ...
                                              /*--- Compute constant quantities
                                                    (mixture density, (specific) total energy, mass fractions, 'mixture' velocity)
                                                    for the updates ---*/
                                              ...
                                              m1_loc*(e1_loc + static_cast<Number>(0.5)*norm2_vel);
