...
namespace samurai {
  /**
    * Implementation of a instantaneous relaxation operator (velocity and then pressure)
    */
  template<class Field>
  class RelaxationVelPresOperator: public Source<Field> {
  public:
    ...
    RelaxationVelPresOperator() = default; /*--- Default constructor (not useful here) ---*/

    RelaxationVelPresOperator(const SG_EOS<Number>& EOS_phase1_,
  ...
  //
  template<class Field>
  RelaxationVelPresOperator<Field>::
  RelaxationVelPresOperator(const SG_EOS<Number>& EOS_phase1_,
  ...
  //
  template<class Field>
  decltype(make_cell_based_scheme<typename RelaxationVelPresOperator<Field>::cfg>())
  RelaxationVelPresOperator<Field>::make_relaxation() {
