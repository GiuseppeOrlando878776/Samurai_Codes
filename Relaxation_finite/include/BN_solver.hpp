...
private:
  ...
  std::unique_ptr<samurai::Source<Field>> source_operator; /*--- Function to compute the treatment of source term (finite-rate or instantaneous)
                                                                 (this is necessary to call 'make_relaxation') ---*/
...
BN_Solver<dim>::BN_Solver(const xt::xtensor_fixed<double, xt::xshape<dim>>& min_corner,
  ...
  path(sim_param.save_dir)
  {
    ...
    /*--- Create the variable for the treatment of source term ---*/
    if(apply_relaxation) {
      if(sim_param.apply_finite_rate_relaxation) {
        if(sim_param.relax_instantaneous_velocity) {
          source_operator = std::make_unique<samurai::FiniteRatePresTemp<Field>>(EOS_phase1, EOS_phase2,
          ...
          source_operator = std::make_unique<samurai::FiniteRatePresTempVel<Field>>(EOS_phase1, EOS_phase2,
                                                                                    ...
                                                                                    sim_param.tau_p,
                                                                                    sim_param.tau_T);
          source_operator->set_source_name("Finite rate (coupled pT, instantaneous velocity)");
        }
      }
      else {
        if(sim_param.relax_velocity) {
          if(sim_param.relax_pressure) {
            if(sim_param.relax_temperature) {
              source_operator = std::make_unique<samurai::RelaxationVelPresTempOperator<Field>>(EOS_phase1, EOS_phase2);
              source_operator->set_source_name("Instantaneous relaxation");
            }
            else {
              source_operator = std::make_unique<samurai::RelaxationVelPresOperator<Field>>(EOS_phase1, EOS_phase2);
              source_operator->set_source_name("Instantaneous velocity and pressure relaxation");
            }
          }
          else {
            source_operator = std::make_unique<samurai::RelaxationVelOperator<Field>>();
            source_operator->set_source_name("Instantaneous velocity relaxation");
          }
        }
        else if(sim_param.relax_pressure) {
          source_operator = std::make_unique<samurai::RelaxationPresOperator<Field>>(EOS_phase1, EOS_phase2);
          source_operator->set_source_name("Instantaneous pressure relaxation");
        }
        else {
          if(rank == 0) {
            std::cerr << "You asked to perform relaxation, but you did not specify a suitable combination" << std::endl;
          }
          exit(1);
        }
      }
...
void BN_Solver<dim>::init_variables(const Riemann_Parameters<Number>& Riemann_param) {
  /*--- Resize fields since now mesh has been surely created ---*/
...
void BN_Solver<dim>::run(const std::size_t nfiles) {
  ...
  /*--- Create the variable for the source term ---*/
  auto Relaxation_operator = source_operator->make_relaxation();
