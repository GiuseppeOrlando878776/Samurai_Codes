...
namespace samurai {
  ...
  auto HLLCFlux<Field>::compute_middle_state(const FluxValue<typename Flux<Field>::cfg>& q,
    ...
    /*--- For transported quantities (alpha1 and Y1), star state coincident with input state ---*/
    const auto rho         = q(M1_INDEX) + q(M2_INDEX);
    const auto alpha1_star = q(RHO_ALPHA1_INDEX)/rho;
    ...
    q_star(RHO_ALPHA1_INDEX)     = rho_star*alpha1_star;
