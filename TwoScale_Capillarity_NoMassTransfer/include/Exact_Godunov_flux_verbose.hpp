    ...
    if(u_star > 0.0) {
      ...
      if(p_star > p_L) {
        ...
        if(!std::isnan(s_L) && s_L < 0.0) {
          ...
          if(q_star(M1_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 1 right of left shock:" << qL(M1_INDEX) << std::endl;
            exit(1);
          }
          q_star(M2_INDEX)         = m2_L_star;
          if(q_star(M2_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 2 right of left shock:" << qL(M2_INDEX) << std::endl;
            exit(1);
          }
          q_star(RHO_ALPHA1_INDEX) = rho_L_star*alpha1_L;
          if(q_star(RHO_ALPHA1_INDEX) < 0.0) {
            std::cerr << "Negative volume fraction phase 1 right of left shock:" << qL(RHO_ALPHA1_INDEX) << std::endl;
            exit(1);
          }
          if(curr_d == 0) {
      ...
      // 3-waves left fan
        ...
        if(sH_L < 0.0 && sT_L > 0.0) {
          ...
          if(q_star(M1_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 1 left fan: " << qL(M1_INDEX) << std::endl;
            exit(1);
          }
          q_star(M2_INDEX)         = m2_L_fan;
          if(q_star(M2_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 2 left fan: " << qL(M2_INDEX) << std::endl;
            exit(1);
          }
          q_star(RHO_ALPHA1_INDEX) = rho_L_fan*alpha1_L;
          if(q_star(RHO_ALPHA1_INDEX) < 0.0) {
            std::cerr << "Negative volume fraction phase 1 left fan: " << qL(RHO_ALPHA1_INDEX) << std::endl;
            exit(1);
          }
          if(curr_d == 0) {
        ...
        // Right of the left fan. Compute the state
          ...
          q_star(M1_INDEX)         = m1_L_star;
          if(q_star(M1_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 1 right of left fan:" << qL(M1_INDEX) << std::endl;
            exit(1);
          }
          q_star(M2_INDEX)         = m2_L_star;
          if(q_star(M2_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 2 right of left fan:" << qL(M2_INDEX) << std::endl;
            exit(1);
          }
          q_star(RHO_ALPHA1_INDEX) = rho_L_star*alpha1_L;
          if(q_star(RHO_ALPHA1_INDEX) < 0.0) {
            std::cerr << "Negative volume fraction phase 1 right of left fan:" << qL(RHO_ALPHA1_INDEX) << std::endl;
            exit(1);
          }
          if(curr_d == 0) {
            q_star(RHO_U_INDEX)     = rho_L_star*u_star;
    ...
    // Right "connecting state"
      ...
      if(p_star > p_R) {
        ...
        // Left of right shock, compute the state
          ...
          if(q_star(M1_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 1 left of right shock:" << qR(M1_INDEX) << std::endl;
            exit(1);
          }
          q_star(M2_INDEX)         = m2_R_star;
          if(q_star(M2_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 2 left of right shock:" << qR(M2_INDEX) << std::endl;
            exit(1);
          }
          q_star(RHO_ALPHA1_INDEX) = rho_R_star*alpha1_R;
          if(q_star(RHO_ALPHA1_INDEX) < 0.0) {
            std::cerr << "Negative volume fraction phase 1 left of right shock:" << qR(RHO_ALPHA1_INDEX) << std::endl;
            exit(1);
          }
          if(curr_d == 0) {
      ...
      // 3-waves right fan
        ...
        // Compute the state in the right fan
          ...
          if(q_star(M1_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 1 right fan:" << qR(M1_INDEX) << std::endl;
            exit(1);
          }
          q_star(M2_INDEX)         = m2_R_fan;
          if(q_star(M2_INDEX) < 0.0) {
            std::cerr << "Negative mass phase 2 right fan:" << qR(M2_INDEX) << std::endl;
            exit(1);
          }
          q_star(RHO_ALPHA1_INDEX) = rho_R_fan*alpha1_R;
          if(q_star(RHO_ALPHA1_INDEX) < 0.0) {
            std::cerr << "Negative volume fraction phase 1 right fan:" << qR(RHO_ALPHA1_INDEX) << std::endl;
            exit(1);
          }
          if(curr_d == 0) {
