...
#include "cases/wave_interface/wave_interface.hpp"
#include "cases/acoustic_wave/acoustic_wave.hpp"
...
make_test_case(const std::string& name_tc,
  ...
  if constexpr (EquationData::dim == 1) {
    if(name_tc == "wave_interface") {
      return std::make_unique<WaveInterface<Traits, AuxFields>>(param_file);
    }
    if(name_tc == "acoustic_wave") {
      return std::make_unique<AcousticWave<Traits, AuxFields>>(param_file);
    }

    throw std::invalid_argument("make_test_case: unknown test case '" + name_tc + "'.\n"
                                "Available cases: wave_interface, acoustic_wave");
  }
