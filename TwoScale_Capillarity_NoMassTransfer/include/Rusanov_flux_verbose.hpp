...
namespace samurai {
  ...
  FluxValue<typename Flux<Field>::cfg> RusanovFlux<Field>::compute_discrete_flux(const FluxValue<typename Flux<Field>::cfg>& qL,
    ...
    if(qL(M1_INDEX) < 0.0) {
      std::cerr << "Negative mass phase 1 left state:" << qL(M1_INDEX) << std::endl;
      exit(1);
    }
    if(qL(M2_INDEX) < 0.0) {
      std::cerr << "Negative mass phase 2 left state:" << qL(M2_INDEX) << std::endl;
      exit(1);
    }
    if(qL(RHO_ALPHA1_INDEX) < 0.0) {
      std::cerr << "Negative volume fraction phase 1 left state:" << qL(RHO_ALPHA1_INDEX) << std::endl;
      exit(1);
    }

    if(qR(M1_INDEX) < 0.0) {
      std::cerr << "Negative mass phase 1 right state:" << qR(M1_INDEX) << std::endl;
      exit(1);
    }
    if(qR(M2_INDEX) < 0.0) {
      std::cerr << "Negative mass phase 2 right state:" << qR(M2_INDEX) << std::endl;
      exit(1);
    }
    if(qR(RHO_ALPHA1_INDEX) < 0.0) {
      std::cerr << "Negative volume fraction phase 1 right state:" << qR(RHO_ALPHA1_INDEX) << std::endl;
      exit(1);
    }
