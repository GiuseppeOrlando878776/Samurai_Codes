...
void TwoScaleCapillarity<dim>::update_geometry() {
  ...
  H = -static_cast<Field_Scalar>(divergence(normal));
}
...
void TwoScaleCapillarity<dim>::init_variables(const double R, const double eps_over_R, const double alpha_residual) {
  ...
  H           = samurai::make_scalar_field<typename Field::value_type>("H", mesh);
  ...
  div_vel       = samurai::make_scalar_field<typename Field::value_type>("div_vel", mesh);
  ...
  H_bar           = samurai::make_scalar_field<typename Field::value_type>("H_bar", mesh);
  ...
  H_bar = -static_cast<Field_Scalar>(divergence(normal_bar));
...
void TwoScaleCapillarity<dim>::execute_postprocess(const double time) {
  ...
  samurai::for_each_cell(mesh,
                         [&](const auto& cell)
                         {
                           ...
                           alpha1_bar_int += alpha1_bar[cell]*std::pow(cell.length, dim);
  ...
  alpha1_integral          << std::fixed << std::setprecision(12) << time << '\t' << alpha1_int          << std::endl;
  ...
  alpha1_d_integral        << std::fixed << std::setprecision(12) << time << '\t' << alpha1_d_int        << std::endl;
  ...
  alpha1_bar_integral      << std::fixed << std::setprecision(12) << time << '\t' << alpha1_bar_int      << std::endl;
