...
double InterfaceInclusion<dim>::get_max_lambda() const {
  ...
  samurai::for_each_cell(mesh,
                         [&](const auto& cell)
                         {
                           ...
                           const auto rho1      = conserved_variables[cell][M1_INDEX]/alpha1;
...
void InterfaceInclusion<dim>::perform_mesh_adaptation() {
  save(fs::current_path(), "_before_mesh_adaptation", conserved_variables, alpha1_bar);
...
//
template<std::size_t dim>
void InterfaceInclusion<dim>::check_data(unsigned int flag) {
  std::string op;
  if(flag == 0) {
  ...
  samurai::for_each_cell(mesh,
                         [&](const auto& cell)
                         {
                           ...
                           if(alpha1_bar[cell] < 0.0) {
                             std::cerr << "Negative effective volume fraction for phase 1 " + op << std::endl;
                             save(fs::current_path(), "_diverged", conserved_variables, alpha1_bar);
                             exit(1);
                           }
                           else if(alpha1_bar[cell] > 1.0) {
                             std::cerr << "Exceeding effective volume fraction for phase 1 " + op << std::endl;
                             save(fs::current_path(), "_diverged", conserved_variables, alpha1_bar);
                             exit(1);
                           }
